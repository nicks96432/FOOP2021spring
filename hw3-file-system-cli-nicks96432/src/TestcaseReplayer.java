...
public class TestcaseReplayer {
	...
	public List<String> replay(List<String> inputs, List<String> outputs) {
		...
		while (!input.equals("exit")) {
			replay.add(String.format("%s> %s", TEXT_CYAN + output.replace("Current path: ", "").trim(),
					TEXT_YELLOW + input));
			output = out.next();
		...
		replay.add(String.format("%s> %s", TEXT_CYAN + output.replace("Current path: ", "").trim(), TEXT_YELLOW + input)
				+ TEXT_RESET);
		return replay;
	}
